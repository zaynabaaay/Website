...
`index.html` is the **homepage** (Phase 5): opening statement, the self-opening demonstration, the Table of Contents sentences, and the path to the Catalogue. Its demonstration waits for an IntersectionObserver before playing — it sits below the fold and only ever plays once, so a load-time timer would play it to an empty screen. "Plays once" is judged by the same `storiel:opened:<id>` key `js/motion.js` writes, so opening it by hand also counts.

`foundations/index.html` is the Phase 1 **living style page** (tokens, type, spacing, motion, both zone physics, the opening-ceremony demo), kept as a founder-facing review page rather than deleted, since the typeface question is still open. It carries founder-facing review tools that are not part of the site itself: a **seal-pair audit** (renders each seal's tint/text pair with contrast ratios computed live from the token values — never hardcode a copy of the palette in that script) and a **typeface comparison** (toggles candidates by overriding `--font-serif` on `:root`, which cascades to the whole page since it's one shared token; it never touches `css/tokens.css`).
...
`catalogue/index.html` (Phase 2) is the Catalogue index, served at `/catalogue/`. Errand zone: normal scrolling, no scroll-snap, no opening ceremony. All four cards link to their product pages.

Each product has a page at a plain-word URL (`/save-the-date/`, `/wedding-invitation/`, `/anniversary/`, `/birthday/`) and a piece under it (`<product>/piece/`). They share `css/product.css` and `css/piece.css`; the per-product seal comes from a `.seal-N-product` class on `<body>` (defined in `tokens.css`) which sets `--product-seal`/`--product-seal-text`. **Never hardcode a `--seal-N` in product or piece CSS** — that is what the class is for, and a page and its piece must carry the same one. Assignments: save-the-date 1, wedding-invitation 2, anniversary 3, birthday 4; seal-5 is free.

`anniversary/index.html` is the reference product page, served at `/anniversary/` — storytelling zone, five spreads, colophon, `Buy — €140`, and the preview entry point. **Every product's name, colophon facts and preview copy were written by the assistant, not the founder** (the founder delegated), so capacities, delivery windows, the ten-year permanence commitment, the prices, and the preview gates are all unconfirmed proposals — see `DECISIONS.md` before treating any of them as settled. Its preview cover reuses the shared `.opening` mechanism.

`<product>/piece/` **is** the product — the actual build a buyer receives (Anniversary Scroll: nine pages). It serves both the preview and (eventually) the purchased piece from one codebase, which §7 requires so the two can never drift: `?gate=N` renders the first N pages plus the gate card, and with no parameter it renders all nine with no gate card. Its story text and `[PHOTOGRAPH]` blocks are **assistant-written sample content**, not founder-authored (§9.8 is still open) — replace them wholesale; nothing in the mechanism depends on them.
...
- `css/product.css` — product-page nav (also used by the homepage), spread content, cover block, seal-marked personalizable list, buy button.
- `css/home.css` / `js/home.js` — homepage spreads, Table of Contents sentences, and the scroll-triggered self-opening demonstration (which triggers the shared mechanism rather than re-implementing it).
